...
"""时钟守卫人工重置工具（用户裁决 2026-08-29：告警 + 显式人工重置）。
...
用法：
    ...
    可选 --env .env：审计密钥来源路径（缺省取 YIBAN_ENV_FILE，
    两者都没有才回落到当前目录 .env——在应用根之外运行请务必显式指定）。
    显式指定时该文件必须已存在（打错路径直接 exit 2，
...
def reset(db_path, env_file=None):
    ...
    env_file：审计密钥来源 .env 路径——本函数经 db.audit() 写哈希链，
