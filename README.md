...
- **普通用户**：邮箱注册后提交自己的易班账号（名称+手机号+密码+设备信息），管理员审核通过后参与每日自动签到；可查看自己账号的签到状态与最近记录；每个用户限提交一个账号；可自助注销账号（两次确认 + 密码验证，3 天内可撤销）
...
| 网站用户（邮箱/密码哈希/角色） | `yiban.db` 的 `users` 表 | scrypt 哈希存储；支持注销（软删除 + 3 天宽限期，v0.20.0+），注销后邮箱可重新注册 |
