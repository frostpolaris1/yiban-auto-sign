...
**容量预估**（「系统设置 → 概览」卡，按当前签到窗口与账号间隔实时估算）：

| 口径 | 估算方式 | 含义 |
|------|---------|------|
| 账号容量 | 按最大间隔（间隔取配置值） | 最坏情况下窗口内可容纳的账号数 |
| 用户容量 | 按间隔中位数（间隔取配置值的一半） | 典型情况下窗口内可容纳的用户数 |

公式：`可容纳数 ≈ (窗口秒数 − 单账号耗时) ÷ (单账号耗时 + 账号间隔) + 1`。占用超过 80% 标红提醒；已超容量后保存新的账号间隔会被拒绝，需清理账号/用户数量或调大间隔。
