...
该规模下当前架构（cron + 单脚本顺序执行 + 队列重试）完全够用，资源需求很低（1 核 1G 小服务器即可）。若预期单站点账号超千级、或需要"不同账号不同签到时间/一天多时段"等能力，架构演进路线见 [docs/scheduler-research-20260814.md](docs/scheduler-research-20260814.md)。
