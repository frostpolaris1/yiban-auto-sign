...
网页顶部/侧边栏的品牌图标默认是**占位符**（内联 SVG 蓝色圆角块 +「签」字，无版权资源，随仓库分发安全）。需要换成自己的图标时，二选一：

- **方式 A（推荐，替换文件）**：把 `web/static/vendor/logo.png` 换成你自己的同名图片（约 64×64 以上即可），并把模板中的占位 `<svg>...</svg>` 改回 `<img src="/static/vendor/logo.png" ...>`；
- **方式 B（仅改代码）**：直接把模板中占位 `<svg>` 的内容改成你自己的内联 SVG。

代码位置：
- 管理端（管理员页面）：`web/templates/index.html` 品牌区（侧边栏顶部，搜 `占位图标` 注释）
- 用户端：`web/templates/user.html` 顶栏（搜 `占位图标` 注释）

> 仓库不含品牌图标文件（原 logo.png 因版权来源不明已移除，避免公开分发风险）；自己部署时放入的图标请确保有使用权。
