...
**日志中出现以下内容即代表签到成功：**
```
[2026-08-02 12:45:01] [INFO] ==== 开始执行签到，共 1 个账号 ====
[2026-08-02 12:45:01] [INFO] 随机延迟 45 秒，避免触发风控...
[2026-08-02 12:45:46] [INFO] [13800138000] 登录成功
[2026-08-02 12:45:47] [INFO] [13800138000] 生成定位: (118.789,32.045) 地址: XX大学
[2026-08-02 12:45:48] [INFO] [13800138000] ✅ 签到成功
[2026-08-02 12:45:48] [INFO] ==== 签到汇总 ====
[2026-08-02 12:45:48] [INFO]   ✅ 13800138000: 签到成功
```
...
签到窗口为 **北京时间 13:00–15:00**（根据服务端返回的签到时间），项目每天在该窗口内执行 **2 次**：

| Cron 表达式 | UTC 时间 | 北京时间 | 用途 |
|------------|---------|---------|------|
| `45 4 * * *` | 04:45 | 12:45 | 首次打卡（预留延迟余量） |
| `30 5 * * *` | 05:30 | 13:30 | 容错重试（若首次已成功，遇"已签到"按成功处理） |

> 两次触发均刻意设在窗口开始前，为 GitHub Actions 的触发延迟（5–30 分钟）留出余量，确保实际执行时仍在 13:00–15:00 窗口内（脚本会按服务端返回的签到时间校验）。
...
on:
  schedule:
    - cron: '45 4 * * *'  # 北京时间 12:45
    - cron: '30 5 * * *'  # 北京时间 13:30
```

**Cron 表达式格式**：`分 时 日 月 周`（UTC 时间，北京时间 = UTC + 8）

> ⚠️ 务必保证触发时间落在签到窗口（13:00–15:00）内并预留延迟余量，否则脚本会因"未在签到时间内"而失败。
