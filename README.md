...
|------------|---------|---------|------------|------|
| `50 22 * * *` | 22:50（前一日） | 06:50 | 约 07:15–07:20 | 签到（ECS 为主力，此为备用） |

> GitHub Actions 的 `schedule` 实际触发通常延迟 20–30 分钟。设为 06:50 触发，实际执行约 07:15–07:20，确保落在签到窗口内并留有 30+ 分钟余量。
>
> ECS 服务器（主力）在 06:40 和 07:10 各执行一次，GitHub Actions 作为备用方案。两者错开避免并发。
...
on:
  schedule:
    - cron: '50 22 * * *'  # 北京时间 06:50
```

**Cron 表达式格式**：`分 时 日 月 周`（UTC 时间，北京时间 = UTC + 8）

> ⚠️ 务必保证触发时间 + 预估延迟（20–30 分钟）落在签到窗口（06:30–07:50）内，否则脚本会因"未在签到时间内"而失败。

> ⏱️ GitHub Actions 的 `schedule` 实际触发时间可能延迟 20–30 分钟（高峰期更久），请勿用于要求精确到分钟的场景。
