...
- 🗄️ **SQLite 数据库存储**：账号与用户数据存于 SQLite（0.17+）——多人同时操作不互相覆盖、手机号全局唯一；数据库结构启动时自动迁移升级；批量操作整体回滚；关键管理操作自动审计留痕（HMAC 防篡改）
...
本项目支持三种部署方式，**推荐使用国内云服务器作为主力签到通道**：
...
本项目是**自托管工具（self-hosted）**，面向个人/小团体自建使用，典型部署场景：
...
# 周六、周日默认都不签到；需要的在网页「系统设置 → 周末签到」开启
...
<details>
<summary>📋 服务器部署分步详解（环境准备 / 上传代码 / 配置账号 / 环境变量 / 运行脚本 / crontab / 手动测试）</summary>
...
# YIBAN_ACCOUNT_GAP_MAX=10   # 账号间隔：相邻请求最小间隔秒数（默认 10，见「账号间隔与容量预估」小节）
# YIBAN_START_DELAY_MAX=60   # 已废弃（v0.29.0）：读取后不再生效，旧 .env 残留可删除
EOF
```
...
添加以下内容（每天 6:31 和 7:10 各执行一次；周六日是否签到由网页「系统设置 → 周六/周日签到」开关控制，周六、周日默认都不签到，开启后才尝试）：

```cron
# 易班自动签到 - 每天执行（周六由 YIBAN_SATURDAY_SIGN 开关控制，默认跳过；周日由 YIBAN_SUNDAY_SIGN 控制，默认跳过）
...
> 部分学校周六日也有签到任务：在网页「系统设置」开启「周六/周日签到」后，相应日期也会尝试签到；若学校该日无需签到，将显示为无需签到。周六、周日默认都不签到（v0.29.0 起）。
...
<details>
<summary>🛠️ 常用运维命令（查看日志 / 清理过期数据 / 手动触发 / crontab / 更新部署）</summary>
...
<details>
<summary>💾 Docker 备份与恢复（加密备份 / 手动 tar / 恢复与安全校验 / 密钥分开存放）</summary>
...
>
> ⚠️ 威胁边界：口令与数据**同机**存放（root crontab/.env）时，
...
<details>
<summary>🔐 安全运维：主管理员权限追回（SSH 重置密码 / 会话失效 / 账号密钥轮换 / 崩溃恢复）</summary>
...
1. SSH 登录服务器，编辑 `/data/.env`（Docker）或 `/opt/yiban-auto-sign/.env`（systemd）：
   ...
   - 写入符合主管理员口令策略的新密码（至少 12 位，且包含大写字母、小写字母、数字、符号中的至少三类，弱口令会拒绝启动）：`YIBAN_ADMIN_PASSWORD=新强密码`；
...
崩溃恢复（按中断点区分；注意「改回旧钥即可恢复」只对提交前的
中断成立）：
...
登录成功留有匿名化 IP 审计（登录失败阈值/普通用户越权 403/密钥轮换/
数据导出同样留痕）。
...
<details>
<summary>🐳 Docker 与宿主部署差异（容器内已自动处理项）</summary>

### 7. 与宿主部署的差异（容器内已自动处理）
- **定时签到**：不再依赖宿主 cron，由容器内 `supervisor` 常驻的 `docker/scheduler.py` 复刻"06:31 首签 + 07:10 补签 + 每日清理"语义。
- **时区**：容器已固定 `Asia/Shanghai`；而窗口与日期判定**本身不依赖宿主时区**——程序统一按北京时间计算（`yiban/clock.py`），宿主是 UTC 也不会算错窗口。
- **安全模型**：nginx 通过 `network_mode: service` 与应用共享网络栈，应用仍只见回环流量、信任的 `X-Forwarded-For` 语义不变（限速/登录锁定按真实 IP 生效）。
- **自定义 Web 图标**：取消 `docker-compose.yml` 中 `yiban` 服务里被注释的 `./logo.png` 挂载，把图标放到仓库根 `logo.png` 即可（不放则显示占位符）。

</details>

<details>
<summary>🗂️ Docker 相关目录/文件说明</summary>
...
#    ⚠️ 主管理员口令策略：至少 12 位，且包含大写字母 / 小写字母 / 数字 / 符号
#    中的至少三类；常见弱口令（admin123 / 123456 / password 等）或不满足策略
#    会拒绝启动（fail-closed）。首次登录后建议在「设置 → 修改管理员账密」
#    重新设置口令：修改会写入哈希并清空 .env 明文，旧会话全部失效，并触发安全告警。
...
- **管理员**：登录后可管理全部账号（添加/编辑/删除/排序/手动签到）、审核普通用户提交的账号、查看签到日志与状态、签到调度（含账号间隔）、健康检查（连通性检测/探针）、消息推送、修改管理员账密；支持暗色主题
...
<details>
<summary>🌐 部署形态：域名根 / 独立子域 / 主站子路径（挂载前缀自动适配与部署契约）</summary>
...
<details>
<summary>🎨 自定义 Web 图标（logo.png 放置与回退占位符）</summary>
...
<details>
<summary>📜 合规文档接入（隐私政策 / 用户协议：文件读取与回退占位）</summary>
...
- **用户管理**：查看注册用户、设为/取消管理员（仅主管理员，单个操作并需二次输入当前管理员密码确认，不支持批量）、重置密码、清空账号、完全删除（支持批量，失败整体回滚；重置密码与删除需二次输入当前管理员密码确认）；权限分级（主管理员 / 注册管理员 / 普通用户）
- **设置**：六卡布局——概览（服务器时间/签到状态/签到窗口/容量预估）、批量操作（含移动端完整表格开关）、签到调度（排序/分布/掐头去尾/账号间隔/窗口/周末签到/用户自选时间片）、全局公告、健康检查（连通性检测 + 探针模式）、消息推送（Webhook 与邮件通知，SMTP 发件列表网页配置）、危险区（暂停签到/暂停注册/修改主管理员密码，仅主管理员）
- **日志与状态**：签到日志实时刷新（手机号打码），支持关键字检索、当日全文显示、按日期查看历史日志与导出（日志按天分文件）；今日各账号状态图标与探针/签到事件记录
...
<details>
<summary>🐙 GitHub Actions 备选部署教程（Fork 仓库 / Secrets 配置 / 启用工作流 / 手动测试 / 定时与延迟 / 资源消耗）</summary>
...
>
> **时区**：Actions 的 runner 是 UTC 主机，但签到窗口与"今天"的判定在程序内一律按**北京时间**计算（`yiban/clock.py`），工作流也额外注入了 `TZ: Asia/Shanghai`——无需你另行配置时区。上面表格里的"UTC 时间"只描述 `cron` 字段如何被 GitHub 解释。
...
<details>
<summary>⚙️ 配置参考：环境变量一览 / 账号间隔与容量预估 / 账号 JSON 格式 / 消息通知渠道 / 代理 / 设备绑定 / 探针模式</summary>
...
| `YIBAN_ACCOUNT_GAP_MAX` | 账号间隔：相邻两次签到请求的最小间隔秒数，自动与手动签到均生效；默认 `10`，`0`=关闭（网页「系统设置 → 签到调度」卡可改，见「账号间隔与容量预估」小节） | 可选 |
| `YIBAN_START_DELAY_MAX` | **已废弃**（v0.29.0 起）：启动延迟由签到调度（分片 + 掐头去尾）取代，读取后不再生效；保留键位仅为兼容旧 `.env`，残留可删除 | 可选 |
...
| `YIBAN_PHONE_CODE` | 设备唯一识别码（App 上报值，原样填写），账号未配置设备信息时全局回退 | 视情况 |
| `YIBAN_NOTIFY_URL` | 通知 webhook URL | 可选 |
| `YIBAN_LOGINFAIL_DAILY_MAX` | 登录失败告警独立推送日额度（默认 `3`，`0`=不限）：web 登录失败告警单独记账，与普通/紧急告警额度分账，避免暴力破解类告警烧完当日额度后，审计链异常等重要告警在手机端被吞掉 | 可选 |
| `YIBAN_BATCH_SIGN_COOLDOWN_SEC` | 手动签到全局冷却秒数（默认 `1800`，`0`=关闭）：批量**与单条**手动签到共用同一冷却，成功触发后冷却窗口内拒绝再次触发，防止被盗会话循环发起真实登录（60 秒同账号防抖与此独立计算） | 可选 |
...
| `YIBAN_SATURDAY_SIGN` | 周六签到开关：`1`=周六也执行，`0`/缺省=周六跳过（v0.29.0 起默认关闭，与周日同语义；网页「系统设置」开关） | 可选 |
...
| `YIBAN_MAX_USERS` / `YIBAN_MAX_ACCOUNTS` | 容量上限（默认 `500` 用户 / `200` 账号；`0`=不限）。用户 = 全部未删除注册用户（含未添加账号的）；账号 = 全部非删除活跃账号（含主管理员直属裸账号）。调小上限不删除存量，只限制新增；网页「系统设置 → 概览」卡可直接设置（主管理员专属） | 可选 |
...
账号间隔指相邻两次签到请求之间的最小间隔（秒），用于降低同一 IP 连续登录触发风控的概率，是 [e003 修复](#q1报错-账号或密码错误e003但密码明明是对的)（真实 App 登录特征）之外的纵深防御。**默认 10 秒**，`0` = 关闭；自动签到与手动签到均生效。两种方式调整：

1. **网页「系统设置 → 签到调度」卡**（推荐）：修改「账号间隔」后保存（修改需主管理员密码确认）
2. **手动编辑 `.env`**（服务器 `/opt/yiban-auto-sign/.env`）：

```bash
# 相邻两次签到请求的最小间隔（秒）；默认 10，0=关闭
YIBAN_ACCOUNT_GAP_MAX=10
```

改完保存即可，cron 下次触发自动生效，无需重启任何服务。

> ⚠️ 启动延迟（`YIBAN_START_DELAY_MAX`，脚本启动后随机等待再开始签到）**已废弃**（v0.29.0 起）：错峰打散由「签到调度」的分片调度与掐头去尾承担，该键读取后不再生效，旧 `.env` 中残留的可删除。

**容量预估**（「系统设置 → 概览」卡，按当前签到窗口与账号间隔实时估算，2026-09-08 单档口径）：

公式：`账号容量 ≈ (有效窗口秒数 − 单账号耗时) ÷ (单账号耗时 + 账号间隔) + 1`，其中有效窗口 = 原始签到窗口 − 掐头去尾裁剪秒数（前后各扣，被裁掉的秒数不参与签到、不占容量）。

- **占用口径 = 活跃账号数**：全部非删除账号（含主管理员直属裸账号，owner='admin'）。「账号容量」约束的是易班请求负载 = 实际参与签到的凭据数，裸账号同样发起请求；已注册但未提交账号的人数单独以「潜在负载」展示。
- **超容量仅警示**：概览卡已用（或含潜在负载）超过容量时标红提示，不阻断任何操作；仅保存更大的账号间隔时若现有活跃账号超过新设置预估容量会被拒绝，可通过缩短账号间隔、延长签到窗口或清理不用的账号解决。

**容量上限**（`YIBAN_MAX_USERS` / `YIBAN_MAX_ACCOUNTS`，主管理员可在概览卡直接设置，`0`=不限）：用户上限管**归属注册**（全部未删除注册用户数），账号上限管**账号存量**（活跃账号数，含裸账号）。调小上限不删除存量用户/账号，只限制新增。
...
> **推荐直接在网页端配置**（现由网页设置页写入）：管理员登录 → 系统设置 → 消息推送 → 邮件通知 → 发件 SMTP 列表，可添加多个发件邮箱按顺序主备切换（网页保存即时生效，无需改文件重启）。下方 `.env` 方式仍然有效：未在网页配置过 SMTP 列表时，作为首个（主）发件条目生效。
...
**如何获取**：在易班 App 的校本化签到页面，用工具页面读出 App 通过 JS 桥接暴露的设备信息
（桥动作 `yiban_device` 的回包含 `appVersion`、`deviceModel` 等字段；App 侧另有
`getUUID()` / `getDeviceInfo()` 供页面调用）：

- 回包中的机型名 → `YIBAN_PHONE_MODEL`
- 设备唯一识别码 → `YIBAN_PHONE_CODE`

> ⚠️ **原样复制，不要手工构造或改动**：这两个值的长度与字符集由易班服务端解释，本项目不做假设
> （早期文档曾写"64 位十六进制""`:` 替换为 `-`"，均无依据，已删除）。改动任何一个都会导致设备绑定校验失败。
>
> ⚠️ 不要把**第三方反欺诈 SDK**（例如易班内集成的数美 `SmAntiFraud`）的 `deviceUniqueId`
> 之类字段当成 `YIBAN_PHONE_CODE`——那不是易班设备绑定用的识别码，填错同样会校验失败。
>
...
- **探针模式**：管理员在「系统设置」手动开启，设定**触发时间**（每天几点）与**触发频率**（每天 / 每 N 天 / **下一次计划时间单次执行·仅一次**）；到点后自动健康检查，异常账号通过邮件预警（管理员收汇总邮件、用户收自己账号的预警），结果写入签到日志（来源标记为探针）。
- **注册时验证账号**：管理员在「系统设置」开启后，用户 / 管理员提交账号时即时验证（只读），验证失败**当场打回**并提示原因；同一手机号短时间内多次密码错误将被临时限制验证并明确提示，防止反复试错导致易班账号被锁定。
...
- 默认关闭：`YIBAN_PROBE_ENABLE=0`（开启 `1`）、`YIBAN_ACCOUNT_VERIFY=0`（开启 `1`）；其中在线校验默认走**同步**（提交时当场返回结论），置 `YIBAN_VERIFY_ASYNC=1` 可改为异步（外呼转后台、结果由前端轮询展示，需界面支持）；`YIBAN_PROBE_TIME=20:00`、`YIBAN_PROBE_INTERVAL_DAYS=1`（正整数=每 N 天，`once`=下一次单次执行）。不配置不影响现有签到。
...
它与并行执行体互补：不是一次性跑完就退出，而是在签到时段内**反复扫描"还没签完"的账号**
并随手接手——学校晚放号、窗口内刚通过审核的账号、被慢账号拖住的、失败待重试的，
都不用等下一轮定时任务。窗口结束后它自己退出。

开它需要**两步**（只做第一步不会有任何进程被拉起来）：

```bash
# 第 1 步：打开开关（网页「系统设置 → 执行体配置」里也有这个开关；直接改 .env 同样有效）
YIBAN_FALLBACK_ENABLE=1
```

```bash
# 第 2 步：在宿主加一条 cron，放在签到窗口开始时（窗口 06:30~07:50 则 06:05 挂上即可）
# 编辑 /etc/cron.d/yiban-sign，追加一行（用户要与 run.sh 同属主，通常是 yiban）：
5 6 * * * yiban /bin/bash /opt/yiban-auto-sign/scripts/yiban-fallback.sh
```

包装脚本 `scripts/yiban-fallback.sh` 会自己读 `.env`（所以开关关掉时它**静默退出**，
不会每 5 分钟发一封 cron 邮件），并把日志并到当天的 `sign-YYYY-MM-DD.log`。
它是常驻的，**不需要** timeout 包裹；与 run.sh 并存是安全的（各自持独立锁文件，
分工交给领取池）。cron 行的细节与模板见该脚本头部注释。

> 上面说的是**服务器（裸机/systemd）部署**。容器部署目前**还没有**对应的常驻方式
> （容器调度器未改动）——容器用户暂时只能手工在容器内跑 `signin sign --fallback`。

**怎么确认在跑**（三条，任选）：

```bash
# ① 看日志：窗口内应有"兜底执行体启动"与每轮的"本轮处理 N 个账号"
grep 兜底 /var/log/yiban/sign-$(date +%F).log

# ② 看心跳文件：内容时间戳应在一两个扫描间隔内（进程被 kill -9 也能靠它识别）
cat /var/log/yiban/fallback-alive.json

# ③ 看接口（主管理员）：fallback.status == "running" 才是真在跑
#    "declared_not_running" = 开关开了但没进程 → 检查上面那条 cron 是否漏加
```

### 怎么知道某个执行体是什么类型

每个执行体在数据库里只留一个身份串（`sign_claims.owner`），三种形态长这样：
...
**身份串是稳定的**：同一台机器上重启执行体**不会换名字**（不复含进程号），所以
"第 1 个并行执行体昨天也签过"这种话立得住；主机后缀是为了**两台机器同时跑时不至于同名**
（同名会导致它们互相认领对方的账号）。

**网页哪里看**：主管理员在「执行体配置」页读 `GET /api/scheduler/executors`——
每个并行执行体带 `role`/`label`（如 `并行执行体 #1`），`fallback` 带开关四态与是否在窗口内，
`activity` 则给出**当天每个执行体各做了多少个**（成功/失败/在飞）。

> **身份串不会出现在网页上**：它含主机名与进程号（属部署信息），接口只回
> **1-based 槽位号**（"第 1 个并行执行体"）；需要追溯具体进程时请在服务器上直接查库。

**历史数据为什么显示为「未标注（旧数据）」**：2026-09-16 之前，兜底执行体与单执行体
用的是**同一个前缀** `exec-`（且身份串里带进程号，重启就换人），库里区分不出来。
那批老记录不会被猜测归类，接口照实回 `unknown` + 标签「未标注（旧数据）」——这不是 bug，
是存量数据的事实。新记录都带各自的前缀，从此可判定。
...
- **网页**：`GET /api/scheduler/executors` 返回每个执行体的出口与角色标签、兜底是否
  在跑（四态）、当天各执行体各做了多少个（`activity`）、实测容量建议值；
  接口文档见仓库内 `docs/dev/api-executors.md`；
...
<details>
<summary>🗄️ 数据存储与备份（SQLite 表结构 / 审计查询与链校验 / 备份与恢复 / 回滚逃生门）</summary>
...
> - 跨版本排查要写 `WHERE action IN ('login','login_ok')`：登录成功审计早期即存在，动作名后来由 `login` 改为 `login_ok`；改名前写入的行（现网旧库与 `/var/backups/yiban-*.tar.gz` 历史备份包内）都叫 `login`，只查新名会把它们整段漏掉。
...
脚本内置队列重试模式，用于处理临时性网络问题或 WAF 拦截：失败账号放回队尾分散重试，同账号两次尝试间隔不小于 60 秒。尝试次数按失败类型分档（每次重试附加 0~30 秒随机延迟）：
...
- 当前时间不在管理员设置的签到时间窗口内
- GitHub Actions 的触发延迟（实测约 55–120 分钟）可能导致实际执行时超出窗口
...
- 脚本会按失败类型自动重试（普通最多 3 次、风控类最多 2 次，密码错误不重试，见 [重试机制](#重试机制)），仍失败则标记为错误
...
- 此前依赖 `js2py` 在本机执行网页 JS 破解该反爬，但 js2py 存在无修复版本的沙箱逃逸漏洞 CVE-2024-28397（公开 PoC 可实现任意代码执行），已移除——默认登录流程已改为 fyiban 同款真实 App 请求特征，正常情况下不会走到该反爬页面
...
<details>
<summary>🧪 测试范围与适配说明（已测试环境 / 运行测试命令 / 未覆盖场景与适配）</summary>
...
<details>
<summary>🤖 AI 生成说明与免责声明（AI 编程声明 / AI 安全免责 / 第三方适配免责）</summary>
