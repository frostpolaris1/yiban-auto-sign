...
- 🖥️ **TUI 面板**（服务器端）：SSH 登录后输入 `yiban` 即可打开——账号列表（序号/状态/名称）、签到日志、模式切换（顺序/并发）、连通性检测、服务器时间一屏掌控
...
>
> ECS 服务器（主力）在 06:40 和 07:10 各执行一次（07:10 仅在 06:40 失败时执行），GitHub Actions 作为备用方案。
...
| `YIBAN_ACCOUNT_GAP_MAX` | 顺序模式账号间随机间隔上限秒数：默认 `0`（关闭）；开启后账号间随机停顿 0~N 秒（并发模式无效果） | 可选 |
...
# 顺序模式账号间随机间隔 0~10 秒（并发模式无效果；删除该行 = 关闭）
...
**容量预估**（最坏情况：所有随机都取最大值，窗口 06:30~07:50、cron 06:40 触发）：
...
**面板布局**：
- **左侧**：账号列表——序号（决定顺序模式打卡顺序）、状态图标（⏳ 准备签到 / ✅ 今日成功 / ❌ 今日失败）、名称、手机号、设备型号
- **右上**：签到日志（最近记录，自动刷新）
- **右下**：设置区——执行模式切换（顺序/并发）、随机延迟开关（启动延迟/账号间隔，各含秒数可调）、连通性检测、服务器时间，均写入 `.env`
...
| `S` | 保存（账号 → `accounts.json`，执行模式 → `.env`） |
| `Q` | 退出 |
...
# YIBAN_ACCOUNT_GAP_MAX=10   # 随机延迟：顺序模式账号间 0~10 秒随机（默认关）
EOF
```
