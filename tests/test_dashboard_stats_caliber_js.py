...
标签：F · 前端与界面守卫
覆盖：数据总览「账号数（`cnt`，按 phone 去重）/ 事件数（`row_cnt`，原始行数）」双口径在 JS 侧的聚合行为，加上各视图取哪一列的静态钉点
对应实现：数据总览页脚本里的 `normalizeDaily`（分布/趋势取列在同一文件）
关键断言：同一账号两行在分布口径只算 1、在事件口径算 2；两个口径必须分开存放且合计不相等；分布视图读 `byStatus`、趋势与成功率读 `.events` 并写明「次」（数字与单位必须对得上）
依赖：⚠ **需要 node 真跑**——`normalizeDaily` 按花括号配对从源码抽出后交给 node 执行（连带补一个最小 `statusKind`），`shutil.which("node")` 取不到时整类 `skipUnless`。三条取列写法的静态源码钉点与它们同在一个类里，所以本机没有 node 时本文件**零用例执行**、不是「还剩静态那几条在跑」。不联网
...
ROWS = [
    # 同一账号当日两行（跳过发生在领取前，每个执行体各写一行）：双口径的最小样本
