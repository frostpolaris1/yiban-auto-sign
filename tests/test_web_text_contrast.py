...
标签：F · 前端与界面守卫
覆盖：「次要文字」的深浅档位不得写反（前提用可执行方式证明 + 源码不再出现写反形态 + 正确形态仍在用），以及签到日历日期格 / 状态行 / 时段按钮 / 主色实心面的配色实数
对应实现：`web/static/css/app.css` 的 `--cal-*` / `--state-*` / `--slot-*` / `--primary-solid` 令牌；扫描对象是 `web/templates` 与 `web/static/js` 里的色对写法
关键断言：正确形态在两种模式下都达 AA、写反形态在两种模式下**都**不达 AA——前提一旦不成立本文件先失败并提示复核，而不是让「照这个换就对了」的规则悄悄失效；反向守卫（正确形态未归零）防一次全局替换把约定清空；深色 `--primary` 必须保持登记档位（浅蓝服务文字/描边/焦点环），白字压主色的面一律走 `--primary-solid`
依赖：纯本地——按主题解析 app.css 后**实算**对比度，扫前端源码原文且不剥注释；不执行 JS、无需 node、不联网
...
CONTRAST_PAIRS = (
    ...
    # 总览·热力图（2026-09-14 §2 语义反转）：未来格=空白（日期数字落卡片底），
    # 仅跳过/无记录格=灰底（--bg-muted）。两套都是既有语义令牌，改令牌值或把
    # 底色语义改回反相都会在这里被重新实测。
    ("总览·热力图·无记录/仅跳过格", "t-base", "bg-muted"),
    ("总览·热力图·未来格（空白，落卡片底）", "t-base", "bg-card"),
)
