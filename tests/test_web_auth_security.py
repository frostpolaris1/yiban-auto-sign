...
依赖：纯本地 Flask test client + 临时 `.env`/SQLite，不联网、不访问真实易班接口；无需 node；并发用例起真线程压计数。`CredentialWriteGateTest` / `RoleHardeningTest` 等把 `YIBAN_PW_GATE` 钉成 `full`，主类 `SecurityFixes021Test` 未设档位（走默认 `risk`）。无需 node
