...
依赖：纯本地 Flask test client + 临时 `.env`/SQLite，不联网、不访问真实易班接口；无需 node；多处 importlib 以独立模块名装载 `web/app.py`（共用模块对象会读到别人的 `.env`）。各基类与多数用例都显式写 `YIBAN_PW_GATE=full`——默认档 `risk` 下这些写操作不再当次要口令。无需 node
