...
（完整链 0→20 跑完，既有 10 张表逐表行数相等，`sign_claims` 只多出 `epoch` 列）。
...
回滚部署（换回旧代码、库不回滚）成立在这条性质上。"不降版本"要与"不抛异常"分开断——
只断前者会放过"抛了但版本没变"，只断后者会放过"静默把版本写回低值"。
每张表都种一行是有意的：空表时整表清空也能过行数比对。
依赖：手工搭库（`_create_tables` + 直接调 v1..v17），不借 `db` 全局连接；
临时目录，无网络、无 skip。
"""
import contextlib
import os
