...
依赖：纯本地 Flask test client + 临时 `.env`/SQLite，不联网、不访问真实易班接口；无需 node，每例新登录会话（不带短时豁免态）。`setUpClass` 另把 `YIBAN_PW_GATE` 钉成 `full`：这些「真变更就要当次口令」的断言是 full 档的样子，默认档 `risk` 下同出口首发不再要口令。无需 node
