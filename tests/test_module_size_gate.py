...
OVERSIZED = {
    ...
    "yiban/engine/schedule.py": (760, (
        ...
        "分派两套公式）与 `executor_count`（K 的唯一口径）后为 675 行。若超过 760 行，按"
        "「配置解析」与「排期算法 + 容量口径」切分：前者是纯读取与回退告警，后者只吃配置"
        "快照，无共享可变状态。"
    )),
    ...
    "yiban/engine/runner.py": (660, (
        ...
        "若超过 660 行，按「命令行装配 + 分支顺序」与「一轮执行 + 收尾汇总」切分，"
        "两半之间只传少数几个值。"
    )),
    ...
    "yiban/store/claims.py": (680, (
        ...
        "加弃权原因分档（result 前缀协议）后为 643 行。"
        "下一步：若超过 680 行，把只读统计族（`states_for_day` / `stats` / `activity` /"
    ...
    "yiban/engine/executor_v3.py": (820, (
        ...
        "心跳后为 756 行。下一步：若超过 820 行，按「通道与尝试闭环（_lane/_attempt/收尾"
    ...
    "scripts/backup.sh": (900, (
        "运维脚本：数据文件一致性快照 + 加密 + 保留策略 + --restore 恢复演练，"
        "头部使用/安装说明与各分支的防护校验占相当篇幅。行数账目按实测：M3 批次0"
        "（MF-76/77/79/80）补上密文回环自检 verify_encrypted_archive、tar 护栏判码、"
        "RETENTION 校验与最近 K 组轮转下界后为 864 行（原上限 800）。非运行时模块，"
        "给硬上限防继续膨胀；若超过 900 行，按「备份」与「恢复/校验」拆两个脚本"
        "（拆分须同步改 cron 安装行、README 与哨兵漂移比对——单文件是既有部署契约）。"
    )),
