...
OVERSIZED = {
    ...
    # 通道数口径）。四种模式共用同一份块划分与容量口径，拆开要把 `_schedule_blocks` 的
    # 块列表与 `_slot_to_bi` 的片号基点跨模块来回传（门禁判据②），而"改一处必漏另一处"
    # 正是这个模块的历史问题（窗口退化的三处告警去重标记就是为此收口的）。它此前已到
    # 598/600，把内联的通道数公式上提为 `channel_count`（M 的唯一口径，供容量公式、
    # 令牌桶突发额度与执行体通道数三处共用）后越线。下一步：若超过 640 行，按
    # 「配置解析（_schedule_config / _env_int / planner_config）」与「排期算法
    # （build_schedule 及其填充助手）」切分——前者是纯读取与回退告警，后者只吃配置快照。
    "yiban/engine/schedule.py": (640, (
        "容量与排期：调度 v2 的 2×2 填充框架 + 窗口判定 + 通道数唯一口径。四种模式共用"
        "同一份块划分与容量口径，拆开要把块列表与片号基点跨模块传（判据②）；把内联的"
        "通道数公式上提为 `channel_count` 后从 598 行越线。若超过 640 行，按「配置解析」"
        "与「排期算法」切分：前者是纯读取与回退告警，后者只吃配置快照，无共享可变状态。"
    )),
    ...
    # 与执行体导入后越线。下一步：若超过 640 行，按「命令行装配 + 分支顺序（main 前半）」
    # 与「一轮执行 + 收尾汇总（main 后半）」切分，两半之间只传 accounts / notify_url /
    # 三道门判定结果这几个值。
    "yiban/engine/runner.py": (640, (
        "入口与轮次编排：分支顺序本身是契约（兜底常驻 → 补签轮判定 → 多执行体派发 → "
        "探针 → 零账号守卫 → --only → --check-config → 周末/暂停门 → 补签轮定向重跑 → "
        "进程锁 → 一轮队列 → 汇总），拆文件会把「顺序」这个唯一事实源分裂成两处。"
        "加调度 v3 的分流点与执行体导入后从 589 行越线。若超过 640 行，按「命令行装配 + "
        "分支顺序」与「一轮执行 + 收尾汇总」切分，两半之间只传少数几个值。"
    )),
    # yiban/engine/executor_v3.py：调度 v3 的执行体核心（M 条通道 + 补货 + 出口限速 +
    # 重试退避 + 终态收尾，外加计划/V/分片集的解析）。
    "yiban/engine/executor_v3.py": (700, (
        "调度 v3 的执行体核心：M 条 asyncio 通道 + 补货 + 出口限速 + 重试退避 + 终态收尾，"
        "以及计划/V/分片集的解析。通道、补货、桶状态落库三者在同一事件循环里共享运行期"
        "上下文（inflight/busy/桶状态），拆开就得把这份共享状态改成跨模块传递"
        "（门禁判据②通信成本高）。规模另来自注释契约（四问头 + 每处并发/收干/脱敏的"
        "「为什么」，约占四分之一）。补上最终放弃通知与顶层异常兜底后从 596 行越线；"
        "放弃留痕与「当日无真实计划行即视为无可用计划」的判据各带一段「为什么」注释，"
        "再增约 35 行。下一步：若超过 700 行，按「通道与尝试闭环（_lane/_attempt/收尾"
        "出口）」与「计划与分片解析（_ensure_plan/_plan_v/_prescan/_mark_window_skips）」"
        "切成两个模块，运行期上下文作为显式入参传递。"
    )),
