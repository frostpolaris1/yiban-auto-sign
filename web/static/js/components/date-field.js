...
(function () {
  ...
  // 判据写成 `v.length >= 2` 而不是 `== 2`，于是一段多出来的 `-D` 会被同一条件一并接住
  // （仍只取前两字段作年、月）。但要说清：全仓只有 setView 写 data-view、且只写两段，模板里
  // 没有预置值、也没有任何持久化（localStorage / 接口字段）会带进三段串，所以"历史三段值"
  // 在本仓没有来源，这条只是防御性容忍，不是给历史值留的兼容口；都拿不到才回落到输入值/今天。
