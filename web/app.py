...
def create_app(host=None):
    ...
    # 两个高危门禁入口登记为 app 属性：它们闭包依赖工厂局部的限速计数表，做成模块级会
    # 跨 app 实例串额度（测试进程反复 create_app）；搬进 web/routes 的通知配置域经
    # web.routes.high_risk_gate() / reconfirm_admin_password() 按实例取回同一闭包。
