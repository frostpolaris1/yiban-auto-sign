...
def create_app(host=None):
    ...
    # 共享会把上一个实例的失败计数与 lock 带进下一个。认证域路由（web/routes/auth.py）
    # 从 extensions 取回**同一份**字典——登录与仍未搬走的改密/注销/恢复路由共用计数，
    # 安全语义依赖同一份账。
    _login_fails = app.extensions["yiban_login_fails"] = {}
    ...
    def api_mail_config_save():
        ...
        smtps（v0.3.4 起）：SMTP 发信条目列表（主备 failover），每条
