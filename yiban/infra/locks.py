...
底层实现交给 `portalocker`（BSD-3-Clause，零强制依赖，纯 Python）。**为什么用库而不是
自己写**：自写版本一次尝试失败就降级为进程内锁——Windows 上 `msvcrt.LK_LOCK` 抢不到锁
约 10 秒后放弃，于是"降级"在多执行体竞争下会真的发生，跨进程互斥静默失效。portalocker
的 `Lock` 带 `timeout`/`check_interval` **重试循环**，会等到超时才报错；它还修过一类自写
很难发现的缺陷（锁的字节区间从任意文件位置起算，导致两个持有者锁的不是同一区间、互斥
实际不成立），并提供共享锁与完备的异常分类。

对外契约（语义**不变**，只是不再依赖各平台实现是否完备）：
