# -*- coding: utf-8 -*-
"""数据层：在线校验任务表 verify_jobs（v15，A4 异步化）。

任务态（排队/在跑/完成/被拒/已取消）与账号审核态（pending/active/rejected）是
两件事，因此独立成表，不污染 accounts.status 枚举。
...
# 超龄收口阈值（秒）：超过它的 pending/running 任务判定为"执行体已死"，
# 收口为 rejected。合法任务的最长寿命是等席位上限 + 看门狗，900s 是其数倍
# ——留足余量，避免把仍在排队等席位的任务误收口。
...
def reclaim_stale(stale_seconds=VERIFY_JOB_STALE_SECONDS, reject_status="",
    ...
    必要性：任务状态由**内存里的线程**推进，进程一旦在 claim 之后、看门狗触发
    之前消失（重启 / 重部署 / OOM），任务就永久停在 running——既不去终态、
    又因 cancel 只认 pending 而无法取消。待办名额是有限的（pending + running），
    卡住的任务持续占用名额，累计到上限后所有新增账号的在线校验永久失败且
    **不会自愈**。故启动期与每次入队前都收口一次。

    判定用 COALESCE(started_at, created_at)：running 看开工时刻、pending 看
    建任务时刻，阈值取 900s（合法寿命上限的数倍）。误收口的代价与既有看门狗
    同性质——任务落 rejected、账号被拒绝，用户可重新提交；时钟被大幅拨快时
    会批量误收口，方向是"少签一次"而非"用错凭据"，与时钟跳变守卫的取舍一致
    （守卫会冻结清理，这里不能冻结，否则功能不自愈）。

    reject_status 非空时，在**同一事务内**对相应账号做 CAS 拒绝（账号状态必须
    ...
    try:
        ...
        # 表可能尚未落地（v15 可选迁移被延后）——此处失败不得影响入队主流程
