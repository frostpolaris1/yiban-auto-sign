...
- **文件心跳与执行体页**：`worker-alive-<slot>.json`（v3 与监督进程都写）。四态：
  ...
  直接调 v3、没有监督进程）时被强杀/超时，或内部未预期异常（`run_executor_v3` 返回 `{}`
  那档）都不写收尾，留"有开始、无收尾"判 `stale`；**受监督多执行体路径**下子进程以退出码
  正常退出（含"本轮失败"的 1，`workers._await_workers` 按 `rc >= 0` 写收尾）即判
  `finished`，失败信息靠退出码与告警体现，不由四态承担。
