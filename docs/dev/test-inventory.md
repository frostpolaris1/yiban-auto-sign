...
| `test_scheduler_gate.py` | 61 | 修复回归测试（对抗性审查 2026-08-29）。 |
...
| `test_session.py` | 39 | 会话缓存表族与会话恢复：有效期判定、凭据加密、重启后恢复。 |
...
| `test_rekey_key_source.py` | 113 | 回归测试（2026-08-29）：密钥来源去 cwd 依赖 + rekey 迁移推送密文。 |
